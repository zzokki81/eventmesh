...
type RelayConfig struct {
	...
	// A larger batch size may improve throughput but can increase latency for individual events.
	BatchSize int `env:"RELAY_BATCH_SIZE" envDefault:"100" validate:"required,min=1"`

	// TickInterval sets the frequency at which the relay wakes up to check for pending events.
	TickInterval time.Duration `env:"RELAY_TICK_INTERVAL" envDefault:"500ms" validate:"required"`

	// MaxAttempts specifies how many times the relay will try to publish an event before marking it as dead.
	MaxAttempts int `env:"RELAY_MAX_ATTEMPTS" envDefault:"5" validate:"required,min=1"`
}
